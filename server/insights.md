...
- **`EvalDashboardService.buildSummary` (`modules/evals/dashboard-service.ts`) originally computed a batch's `status` via `batchRegistry.isRunning(agent.id)` — true for EVERY batch summary of that agent while ANY batch is in flight, not just the one actually running.** `batch-registry.ts`'s module-scope `Map<agentId, batchId>` correctly tracks WHICH batch is running per agent (`runningBatchId(agentId)` already existed as an export), but `buildSummary` is called once per historical batch in a loop (`agentDashboard`'s `for (const batchId of batchIds)`) and used the coarser `isRunning(agentId)` boolean instead of comparing against the specific `batchId` being summarized — so starting a second batch for an agent retroactively relabeled every older, already-complete batch in that agent's history as `"running"` too. Symptom: the Agent Editor's Evals tab and the `/eval/[agentId]` dashboard visibly disagreed on whether anything was running, and the batch-history table showed multiple simultaneously-"Running" rows for batches whose `cases_passed`/metrics were already fully populated. Found via manual UI testing, not by any test — this exact case (2+ batches for one agent, one genuinely in flight) was never exercised by `evals-runs.it.test.ts` before (now added) NOR could it have run in this sandbox (Docker/testcontainers unavailable), and the hermetic suite doesn't touch `dashboard-service.ts` at all. Fix: `batchRegistry.runningBatchId(agent.id) === batchId`. ⇒ Any module-scope "what's currently running" registry keyed coarser than the entity being rendered (here: per-agent, but consumed per-batch) is a trap the moment more than one instance of the finer-grained entity can exist per coarse key — grep for every `isRunning`/similar boolean-only registry check and confirm it's actually being compared against the specific id in scope, not just checked for "any."

  **2026-08-05 same-session follow-up — fixing the above surfaced a second, related gap: a batch registered in `batchRegistry` (i.e. `tryAcquire` already succeeded, a second trigger correctly 409s per AC-23) is INVISIBLE to both dashboards until its first `eval_runs` row lands.** `agentDashboard`/`workspaceDashboard` build every `EvalBatchSummary` exclusively from `runsForBatch`'s persisted rows (`if (runs.length === 0) continue;`) — a real case run through `runAgentReview` can take tens of seconds (measured: 83.6s for one `deepseek-v4-flash` call via OpenRouter in this exact session), so for that whole window the UI shows nothing "running" anywhere while the server is legitimately rejecting a second trigger — a user re-clicking "Run eval" during that window (a very natural thing to do when nothing visibly indicates a run is in progress) gets a confusing, seemingly-causeless "already running" error. Fixed with `EvalDashboardService.runningPlaceholder()`: a synthesized zero-progress `EvalBatchSummary` (`cases_passed: 0`, all `_na: true`) used ONLY to backfill `recent_batches`/`last_batch` when `batchRegistry.runningBatchId(agentId)` names a batch not yet present in the row-derived `summaries` — deliberately NOT folded into `current`/`delta`/`trend`, which stay anchored to the last real batch so headline metrics don't flicker to N/A the instant a new run starts. ⇒ Any "in-progress" UI state backed by a registry-plus-persisted-rows pair has this same latent gap between "registered" and "first persisted evidence" — the registry event (trigger accepted) and the first queryable row are not simultaneous, and real work (LLM calls, external APIs) can make that gap arbitrarily long; the UI must be able to represent "started, no results yet" as a first-class state, not simply be absent until the first row exists.
