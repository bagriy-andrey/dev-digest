...
import type { Brief, Finding, Intent, RunSummary, RunTrace } from '@devdigest/shared';
...
export class ReviewRepository {
  ...
  upsertBrief(prId: string, brief: Brief): Promise<void> {
    return pullRepo.upsertBrief(this.db, prId, brief);
  }

  getBrief(prId: string): Promise<Brief | undefined> {
  ...
  createAgentRun(values: {
    ...
    model: string | null;
    /** Multi-agent group this run belongs to, if started as part of one. */
  ...
  completeAgentRun(
    ...
    values: {
      ...
      tokensIn: number;
      tokensOut: number;
      /** USD cost; null when the provider reported none / on failed runs. */
      costUsd?: number | null;
