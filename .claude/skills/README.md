...
| [pr-self-review](pr-self-review/SKILL.md) | Meta | Pre-PR gate — routes diff through architecture/quality skills, blocks on CRITICAL |
| [sdd-build](sdd-build/SKILL.md) | Meta | Builds an already-approved Implementation Plan: implementer(s) → verify → bounded architecture fix-loop (no spec-creator/planner) |
| [workflow-retro](workflow-retro/SKILL.md) | Meta | Retrospective over a just-completed multi-agent workflow — agent roster/order, token/cost per agent, clarification friction, duplicated work, recommendations. Run manually with `/workflow-retro` |
